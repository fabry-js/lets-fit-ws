...
const OrdiniRecenti: React.FC<OrdiniRecentiProps> = ({isOpen, onClose}) => {
  const ordiniRecentiRef = _firestore.collection("/orders").orderBy("createdAt").limit(100);
  ...
  return (
    <Modal size="full" onClose={onClose} isOpen={isOpen} scrollBehavior="inside">
      ...
      <ModalContent>
        ...
        <ModalCloseButton />
        <ModalBody>
          {filteredRecentOrders ? filteredRecentOrders.map((order: any, id) => {
            const { allIngredients, paymentMethod, restaurantName, totale } = order;
            return (
              <RiepilogoCard
